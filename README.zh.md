...
四个原则，集中在一个文件中，再配合一个五步可执行工作流，帮助编码智能体在真实开发任务中落实这些原则：
...
对于非琐碎的编码任务，这份指南要求智能体遵循一个紧凑协议：

1. **Assumption Check** — 在编辑前说明预期结果、范围、约束和成功标准。
2. **Minimal Plan** — 给出 2-5 个具体步骤，每一步都带验证方式。
3. **Change Boundary** — 说明会触碰什么，以及哪些相邻代码会保持不动。
4. **Simplicity Gate** — 拒绝未被要求的抽象、框架、配置、兼容层或推测性功能。
5. **Verification Contract** — 完成时报告运行了什么、什么通过了、什么没有验证，以及剩余风险。

这样既有足够结构来避免高代价错误，又不会让日常开发变得繁琐。
...
- **编辑前有范围清晰的计划** —— 非琐碎任务会先说明假设、边界和验证检查
- **完成时有验证证据** —— 最终回复会列出已验证内容和剩余风险
